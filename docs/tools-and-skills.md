...
現在のCLIは、TOMLで有効化されたRedmine担当者を順番に確認し、担当している未完了チケットを1件ずつ取得する。プロフィールごとのモデル、endpoint、資格情報、任意のsystem promptを使い、チケット本文とコメント履歴を読んで利用可能なskillまたはtoolを選択して実行する。
